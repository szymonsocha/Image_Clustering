...
```{r, echo=FALSE}
pie_data <- results[results$type %in% c("Non green urban area", "Green space"),]
font = list(family="Courier New", size = 13)
font_tit = list(family="Courier New", color = "black", size = 25)
pie <- plot_ly(data = pie_data,
               type = "pie",
               labels = pie_data$type,
               values = pie_data$Freq,
               textposition = "inside",
               textinfo = "label+percent",
               textfont = font,
               hoverinfo = pie_data$Freq,
               marker = list(colors  = pie_data$colors))
pie <- pie %>% layout(title = list(text = "Green urban area in Warsaw", y = 0.95, font = font_tit),
                      showlegend = F,
                      margin = list(t= 90, r = 60))
pie
```
